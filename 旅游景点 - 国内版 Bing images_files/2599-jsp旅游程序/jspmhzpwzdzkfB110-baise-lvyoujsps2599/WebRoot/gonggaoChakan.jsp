...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/gonggaoMana.action?d=1";
           ...
           function gonggaoDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/gonggaoDel.action?id="+id+"&path=gonggaoChakan.action";
               }
           }
           //修改信息
           function gonggaoGet(id)
           {
                   window.location.href="gonggaoGet.action?id="+id;
           }
			//详细信息
			function gonggaoDetail(id)
			{
				window.location.href="gonggaoDetail.action?id="+id;
			}
           //图片预览
           ...
           function go(index)
           {
               ...
               document.form123.action="gonggaoChakan.action"
               document.form123.submit();
           }
            function gonggaoSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="gonggaoSet.action?id="+id+"&path=gonggaoChakan.action";
				   document.formset.submit();
				   //window.location.href="<%=path %>/gonggaoSet.action?id="+id+"&isurl=1"+"&"+property+"="+value;
               }
           }
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()" leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				<%--<form action="<%=path %>/gonggaoSearchChakan.action" name="formSearch" id="formSearch" method="post">
...
内容:<input type="text" name="neirong" id="neirong" size="15"/>&nbsp;&nbsp;
				...
				<tr bgcolor="#E7E7E7">
					<td height="14" colspan="6" background="<%=path %>/img/tbg.gif">&nbsp;&nbsp;公告新闻管理</td>
				</tr>
				<%--colunmRemarks--%>
				<tr align="center" bgcolor="#FAFAF1" height="22">
				 ...
				 <s:iterator value="#request.page.data" id="gonggao" status="s"> 
				 <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					<td bgcolor="#FFFFFF" align="center">
					 ...
					 <%--colunmNameValue--%>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#gonggao.biaoti.length()>16">
							<s:property value="#gonggao.biaoti.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
					...
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#gonggao.neirong.length()>16">
							<s:property value="#gonggao.neirong.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#gonggao.neirong"/>
						</s:else>
					</td>

					<td width='90' align='center'>
						<a href='<%=path %>/<s:property value="#gonggao.tupian"/>' target='_blank'>
							<img
									src='<%=path %>/<s:property value="#gonggao.tupian"/>' width=88 height=99 border=0/>
						</a>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#gonggao.shijian.length()>16">
							<s:property value="#gonggao.shijian.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
					...
					<td bgcolor="#FFFFFF" align="center">
						<%-- <input type="button" value="审核" onclick="gonggaoSet(${gonggao.id},'shenhe','yes')"/> --%>
						<%-- <input type="button" value="取消审核" onclick="gonggaoSet(${gonggao.id},'shenhe','no')"/> --%>

						<%--<input type="button" value="修改" onclick="gonggaoGet(<s:property value="#gonggao.id"/>)"/>--%>
						<%--<input type="button" value="审核" onclick="gonggaoSet(<s:property value="#gonggao.id"/>,'shenhe','yes')"/>--%>
						<input type="button" value="详情" onclick="gonggaoDetail(<s:property value="#gonggao.id"/>)"/>
						<%--<input type="button" value="删除" onclick="gonggaoDel(<s:property value="#gonggao.id"/>)"/>--%>
						<%--sonTable本表子表管理--%>

<%--
						<input type="button" value="图片管理" onclick="tupianGetBygonggao_id(<s:property value="#gonggao.id"/>)"/>
--%>
					</td>
				</tr>
				 </s:iterator>
			...
			<table width='98%'  border='0'style="margin-top:8px;margin-left: 8px;">
			  <tr>
			    <td>
			      <%--<input type="button" value="添加" style="width: 120px;" onclick="gonggaoAdd()" />--%>
