...
<head>
	...
	<script language="javascript">
		...
		function daochu()
		{
			var url="<%=path %>/lianjieAll.action?d=1";
		...
		//添加信息
		function lianjieAddQian()
		{
			<c:if test="${sessionScope.user==null}">
			alert("请先登录");
			</c:if>
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/lianjie/lianjieAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		...
		function go(index)
		{
			...
			document.form123.action="lianjieManaQian.action"
			document.form123.submit();
		}
		function lianjieAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="lianjieSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>友情链接</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/lianjieSearchQian.action">

				            名称:
            <input type="text" name="name" id="name" style='border:solid 1px #000000; color:#666666' size="12" />
链接地址:<input type="text" name="url" id="url" size="15"/>
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<tr>
					...
					<%--colunmRemarks--%>
									    <td bgcolor='#CCFFFF' align="center" >名称</td>
				    <td bgcolor='#CCFFFF' align="center" >链接地址</td>
				...
				<c:forEach items="${requestScope.page.data}" var="lianjie" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(lianjie.name)>'16'}">
							${fn:substring(lianjie.name,0,16)}...
						</c:if>
						<c:if test="${fn:length(lianjie.name)<='16'}">
							${lianjie.name}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(lianjie.url)>'16'}">
							${fn:substring(lianjie.url,0,16)}...
						</c:if>
						<c:if test="${fn:length(lianjie.url)<='16'}">
							${lianjie.url}
						</c:if>
						...
						<td width="30" align="center">
							<a href="<%=path %>/lianjieDetailQian.action?id=${lianjie.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="lianjieAddQian()" />
							--%>
