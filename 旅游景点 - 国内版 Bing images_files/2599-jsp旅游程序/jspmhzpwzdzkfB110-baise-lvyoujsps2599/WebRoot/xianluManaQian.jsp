...
<head>
	...
	<script language="javascript">
		...
		function daochu()
		{
			var url="<%=path %>/xianluAll.action?d=1";
		...
		//添加信息
		function xianluAddQian()
		{
			<c:if test="${sessionScope.user==null}">
			alert("请先登录");
			</c:if>
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/xianlu/xianluAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		...
		function go(index)
		{
			...
			document.form123.action="xianluManaQian.action"
			document.form123.submit();
		}
		function xianluAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="xianluSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>旅游路线</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/xianluSearchQian.action">
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<tr>
				    ...
				    <td bgcolor='#CCFFFF' align="center" >旅游路线类别</td>
				    <td bgcolor='#CCFFFF' align="center" >附件</td>
				    <td bgcolor='#CCFFFF' align="center" >时间</td>
				    <td bgcolor='#CCFFFF' align="center" >旅游路线用户</td>
				...
				<c:forEach items="${requestScope.page.data}" var="xianlu" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(xianlu.biaoti)>'16'}">
							${fn:substring(xianlu.biaoti,0,16)}...
						</c:if>
						<c:if test="${fn:length(xianlu.biaoti)<='16'}">
							${xianlu.biaoti}
						</c:if>
					...
					<%--<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(xianlu.neirong)>'16'}">
							${fn:substring(xianlu.neirong,0,16)}...
						</c:if>
						<c:if test="${fn:length(xianlu.neirong)<='16'}">
							${xianlu.neirong}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(xianlu.leibie.mingcheng)>'16'}">
							${fn:substring(xianlu.leibie.mingcheng,0,16)}...
						</c:if>
						<c:if test="${fn:length(xianlu.leibie.mingcheng)<='16'}">
							${xianlu.leibie.mingcheng}
						</c:if>
					</td>

					<td width='90'>
						<a href='<%=path %>/${xianlu.image}' target='_blank'>
							<img src='<%=path %>/${xianlu.image}'
																							width=88 height=99
																							border=0/>
						</a>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(xianlu.shijian)>'16'}">
							${fn:substring(xianlu.shijian,0,16)}...
						</c:if>
						<c:if test="${fn:length(xianlu.shijian)<='16'}">
							${xianlu.shijian}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(xianlu.users.loginname)>'16'}">
							${fn:substring(xianlu.users.loginname,0,16)}...
						</c:if>
						<c:if test="${fn:length(xianlu.users.loginname)<='16'}">
							${xianlu.users.loginname}
						</c:if>
					</td>
						...
						<td width="30" align="center">
							<a href="<%=path %>/xianluDetailQian.action?id=${xianlu.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="xianluAddQian()" />
							--%>
