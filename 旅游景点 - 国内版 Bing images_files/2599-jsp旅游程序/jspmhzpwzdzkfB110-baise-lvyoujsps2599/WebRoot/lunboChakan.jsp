...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/lunboMana.action?d=1";
           ...
           function lunboAdd()
           {
              var url="<%=path %>/admin/lunbo/lunboAdd.jsp";
              window.location.href=url;
           }
           //删除信息
           function lunboDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/lunboDel.action?id="+id+"&path=lunboChakan.action";
               }
           }
           //修改信息
           ...
           function go(index)
           {
               ...
               document.form123.action="lunboChakan.action"
            ...
            function lunboSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="lunboSet.action?id="+id+"&path=lunboChakan.action";
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()" leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				<%--<form action="<%=path %>/lunboSearchChakan.action" name="formSearch" id="formSearch" method="post">
				...
				<tr align="center" bgcolor="#FAFAF1" height="22">
				 ...
				 <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					...
					<td bgcolor="#FFFFFF" align="center">
						<%-- <input type="button" value="审核" onclick="lunboSet(${lunbo.id},'shenhe','yes')"/> --%>
						<%-- <input type="button" value="取消审核" onclick="lunboSet(${lunbo.id},'shenhe','no')"/> --%>

						<%--<input type="button" value="修改" onclick="lunboGet(<s:property value="#lunbo.id"/>)"/>--%>
						<%--<input type="button" value="审核" onclick="lunboSet(<s:property value="#lunbo.id"/>,'shenhe','yes')"/>--%>
...
<%--
						<input type="button" value="图片管理" onclick="tupianGetBylunbo_id(<s:property value="#lunbo.id"/>)"/>
--%>
					</td>
				</tr>
				 </s:iterator>
