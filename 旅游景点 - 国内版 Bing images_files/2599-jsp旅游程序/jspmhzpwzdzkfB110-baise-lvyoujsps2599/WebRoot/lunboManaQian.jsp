...
<head>
	...
	<script language="javascript">
		...
		function daochu()
		{
			var url="<%=path %>/lunboAll.action?d=1";
		...
		//添加信息
		function lunboAddQian()
		{
			<c:if test="${sessionScope.user==null}">
			alert("请先登录");
			</c:if>
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/lunbo/lunboAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		function lunboDel(id)
		{
			if(confirm('您确定删除吗?'))
			{
				window.location.href="<%=path %>/lunboDel.action?id="+id;
			}
		}
		//修改信息
		...
		function go(index)
		{
			...
			document.form123.action="lunboManaQian.action"
			document.form123.submit();
		}
		function lunboAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="lunboSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>轮播图</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/lunboSearchQian.action">
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<c:forEach items="${requestScope.page.data}" var="lunbo" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(lunbo.biaoti)>'16'}">
							${fn:substring(lunbo.biaoti,0,16)}...
						</c:if>
						<c:if test="${fn:length(lunbo.biaoti)<='16'}">
							${lunbo.biaoti}
						</c:if>
					...
					<td width='90'>
						<a href='<%=path %>/${lunbo.image}' target='_blank'>
							<img src='<%=path %>/${lunbo.image}'
																							width=88 height=99
																							border=0/>
						</a>
					</td>
						...
						<td width="30" align="center">
							<a href="<%=path %>/lunboDetailQian.action?id=${lunbo.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="lunboAddQian()" />
							--%>
