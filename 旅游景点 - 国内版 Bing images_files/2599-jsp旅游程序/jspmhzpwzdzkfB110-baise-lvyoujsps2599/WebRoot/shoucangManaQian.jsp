...
<head>
	...
	<script language="javascript">
		...
		function daochu()
		{
			var url="<%=path %>/shoucangAll.action?d=1";
		...
		//添加信息
		function shoucangAddQian()
		{
			<c:if test="${sessionScope.user==null}">
			alert("请先登录");
			</c:if>
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/shoucang/shoucangAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		function shoucangDel(id)
		{
			if(confirm('您确定删除吗?'))
			{
				window.location.href="<%=path %>/shoucangDel.action?id="+id;
			}
		}
		//修改信息
		...
		function go(index)
		{
			...
			document.form123.action="shoucangManaQian.action"
			document.form123.submit();
		}
		function shoucangAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="shoucangSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>收藏</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/shoucangSearchQian.action">
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<tr>
				    ...
				    <td bgcolor='#CCFFFF' align="center" >收藏景点</td>
				    <td bgcolor='#CCFFFF' align="center" >收藏用户</td>
				...
				<c:forEach items="${requestScope.page.data}" var="shoucang" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(shoucang.shijian)>'16'}">
							${fn:substring(shoucang.shijian,0,16)}...
						</c:if>
						<c:if test="${fn:length(shoucang.shijian)<='16'}">
							${shoucang.shijian}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(shoucang.jingdian.biaoti)>'16'}">
							${fn:substring(shoucang.jingdian.biaoti,0,16)}...
						</c:if>
						<c:if test="${fn:length(shoucang.jingdian.biaoti)<='16'}">
							${shoucang.jingdian.biaoti}
						</c:if>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(shoucang.users.loginname)>'16'}">
							${fn:substring(shoucang.users.loginname,0,16)}...
						</c:if>
						<c:if test="${fn:length(shoucang.users.loginname)<='16'}">
							${shoucang.users.loginname}
						</c:if>
					</td>
						...
						<td width="30" align="center">
							<a href="<%=path %>/shoucangDetailQian.action?id=${shoucang.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="shoucangAddQian()" />
							--%>
