...
<head>
	...
	<script language="javascript">
		...
		function daochu()
		{
			var url="<%=path %>/jiudianAll.action?d=1";
		...
		//添加信息
		function jiudianAddQian()
		{
			<c:if test="${sessionScope.user==null}">
			alert("请先登录");
			</c:if>
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/jiudian/jiudianAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		...
		function go(index)
		{
			...
			document.form123.action="jiudianManaQian.action"
			document.form123.submit();
		}
		function jiudianAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="jiudianSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>酒店</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/jiudianSearchQian.action">
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<c:forEach items="${requestScope.page.data}" var="jiudian" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(jiudian.biaoti)>'16'}">
							${fn:substring(jiudian.biaoti,0,16)}...
						</c:if>
						<c:if test="${fn:length(jiudian.biaoti)<='16'}">
							${jiudian.biaoti}
						</c:if>
					...
					<%--<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(jiudian.neirong)>'16'}">
							${fn:substring(jiudian.neirong,0,16)}...
						</c:if>
						<c:if test="${fn:length(jiudian.neirong)<='16'}">
							${jiudian.neirong}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(jiudian.dizhi)>'16'}">
							${fn:substring(jiudian.dizhi,0,16)}...
						</c:if>
						<c:if test="${fn:length(jiudian.dizhi)<='16'}">
							${jiudian.dizhi}
						</c:if>
					...
					<td width='90'>
						<a href='<%=path %>/${jiudian.image}' target='_blank'>
							<img src='<%=path %>/${jiudian.image}'
																							width=88 height=99
																							border=0/>
						</a>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(jiudian.shijian)>'16'}">
							${fn:substring(jiudian.shijian,0,16)}...
						</c:if>
						<c:if test="${fn:length(jiudian.shijian)<='16'}">
							${jiudian.shijian}
						</c:if>
						...
						<td width="30" align="center">
							<a href="<%=path %>/jiudianDetailQian.action?id=${jiudian.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="jiudianAddQian()" />
							--%>
