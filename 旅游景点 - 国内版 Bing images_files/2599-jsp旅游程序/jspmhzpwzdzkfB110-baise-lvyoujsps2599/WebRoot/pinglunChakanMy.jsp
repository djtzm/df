...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/pinglunMana.action?d=1";
           ...
           function pinglunDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/pinglunDel.action?id="+id+"&path=pinglunChakanMy.action";
               }
           }
           //修改信息
           function pinglunGet(id)
           {
                   window.location.href="pinglunGet.action?id="+id;
           }
			//详细信息
			function pinglunDetail(id)
			{
				window.location.href="pinglunDetail.action?id="+id;
			}
           //图片预览
           ...
           function go(index)
           {
               ...
               document.form123.action="pinglunChakanMy.action"
               document.form123.submit();
           }
            function pinglunSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="pinglunSet.action?id="+id+"&path=pinglunChakanMy.action";
				   document.formset.submit();
				   //window.location.href="<%=path %>/pinglunSet.action?id="+id+"&isurl=1"+"&"+property+"="+value;
               }
           }
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()" leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				<form action="<%=path %>/pinglunChakanMy.action" name="formSearch" id="formSearch" method="post">
...
内容:<input type="text" name="neirong" id="neirong" size="15"/>&nbsp;&nbsp;
				...
				<tr bgcolor="#E7E7E7">
					<td height="14" colspan="6" background="<%=path %>/img/tbg.gif">&nbsp;&nbsp;评论管理</td>
				</tr>
				<tr align="center" bgcolor="#FAFAF1" height="22">
				 ...
				 <s:iterator value="#request.page.data" id="pinglun" status="s"> 
				 <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					<td bgcolor="#FFFFFF" align="center">
					 ...
					 <%--colunmNameValue--%>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#pinglun.biaoti.length()>16">
							<s:property value="#pinglun.biaoti.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#pinglun.biaoti"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#pinglun.neirong.length()>16">
							<s:property value="#pinglun.neirong.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#pinglun.neirong"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#pinglun.shijian.length()>16">
							<s:property value="#pinglun.shijian.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#pinglun.shijian"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#pinglun.jingdian.biaoti.length()>16">
							<s:property value="#pinglun.jingdian.biaoti.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#pinglun.jingdian.biaoti"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#pinglun.users.loginname.length()>16">
							<s:property value="#pinglun.users.loginname.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#pinglun.users.loginname"/>
						</s:else>
					</td>


					<td bgcolor="#FFFFFF" align="center">
						<%-- <input type="button" value="审核" onclick="pinglunSet(${pinglun.id},'shenhe','yes')"/> --%>
						<%-- <input type="button" value="取消审核" onclick="pinglunSet(${pinglun.id},'shenhe','no')"/> --%>

						<%--<input type="button" value="修改" onclick="pinglunGet(<s:property value="#pinglun.id"/>)"/>--%>
						<%--<input type="button" value="审核" onclick="pinglunSet(<s:property value="#pinglun.id"/>,'shenhe','yes')"/>--%>
						<input type="button" value="详情" onclick="pinglunDetail(<s:property value="#pinglun.id"/>)"/>
						<%--<input type="button" value="删除" onclick="pinglunDel(<s:property value="#pinglun.id"/>)"/>--%>
						<%--sonTable本表子表管理--%>

<%--
						<input type="button" value="图片管理" onclick="tupianGetBypinglun_id(<s:property value="#pinglun.id"/>)"/>
--%>
					</td>
				</tr>
				 </s:iterator>
			...
			<table width='98%'  border='0'style="margin-top:8px;margin-left: 8px;">
			  <tr>
			    <td>
			      <%--<input type="button" value="添加" style="width: 120px;" onclick="pinglunAdd()" />--%>
