...
<head>
	...
	<script language="javascript">
		...
		function daochu()
		{
			var url="<%=path %>/leibieAll.action?d=1";
		...
		//添加信息
		function leibieAddQian()
		{
			<c:if test="${sessionScope.user==null}">
			alert("请先登录");
			</c:if>
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/leibie/leibieAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		...
		function go(index)
		{
			...
			document.form123.action="leibieManaQian.action"
			document.form123.submit();
		}
		function leibieAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="leibieSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>类别</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/leibieSearchQian.action">

				            名称:
            <input type="text" name="mingcheng" id="mingcheng" style='border:solid 1px #000000; color:#666666' size="12" />
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<c:forEach items="${requestScope.page.data}" var="leibie" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(leibie.mingcheng)>'16'}">
							${fn:substring(leibie.mingcheng,0,16)}...
						</c:if>
						<c:if test="${fn:length(leibie.mingcheng)<='16'}">
							${leibie.mingcheng}
						</c:if>
						...
						<td width="30" align="center">
							<a href="<%=path %>/leibieDetailQian.action?id=${leibie.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="leibieAddQian()" />
							--%>
