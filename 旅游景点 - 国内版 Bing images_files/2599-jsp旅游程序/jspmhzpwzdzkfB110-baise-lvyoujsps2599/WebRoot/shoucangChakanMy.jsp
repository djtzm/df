...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/shoucangMana.action?d=1";
           ...
           {
              var url="<%=path %>/admin/shoucang/shoucangAdd.jsp";
              window.location.href=url;
           }
           //删除信息
           function shoucangDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/shoucangDel.action?id="+id+"&path=shoucangChakanMy.action";
               }
           }
           //修改信息
           ...
           function go(index)
           {
               ...
               document.form123.action="shoucangChakanMy.action"
            ...
            function shoucangSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="shoucangSet.action?id="+id+"&path=shoucangChakanMy.action";
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()" leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				<form action="<%=path %>/shoucangChakanMy.action" name="formSearch" id="formSearch" method="post">
				...
				<tr align="center" bgcolor="#FAFAF1" height="22">
				 ...
				 <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					...
					<td bgcolor="#FFFFFF" align="center">
						<%-- <input type="button" value="审核" onclick="shoucangSet(${shoucang.id},'shenhe','yes')"/> --%>
						<%-- <input type="button" value="取消审核" onclick="shoucangSet(${shoucang.id},'shenhe','no')"/> --%>

						<%--<input type="button" value="修改" onclick="shoucangGet(<s:property value="#shoucang.id"/>)"/>--%>
						<%--<input type="button" value="审核" onclick="shoucangSet(<s:property value="#shoucang.id"/>,'shenhe','yes')"/>--%>
...
<%--
						<input type="button" value="图片管理" onclick="tupianGetByshoucang_id(<s:property value="#shoucang.id"/>)"/>
--%>
					</td>
				</tr>
				 </s:iterator>
