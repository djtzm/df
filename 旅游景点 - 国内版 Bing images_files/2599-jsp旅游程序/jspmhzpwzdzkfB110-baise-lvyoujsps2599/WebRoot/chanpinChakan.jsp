...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/chanpinMana.action?d=1";
           ...
           function chanpinAdd()
           {
              var url="<%=path %>/admin/chanpin/chanpinAdd.jsp";
              window.location.href=url;
           }
           //删除信息
           function chanpinDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/chanpinDel.action?id="+id+"&path=chanpinChakan.action";
               }
           }
           //修改信息
           function chanpinGet(id)
           {
                   window.location.href="chanpinGet.action?id="+id;
           }
			//详细信息
			function chanpinDetail(id)
			{
				window.location.href="chanpinDetail.action?id="+id;
			}
           //图片预览
           ...
           function go(index)
           {
               ...
               document.form123.action="chanpinChakan.action"
               document.form123.submit();
           }
            function chanpinSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="chanpinSet.action?id="+id+"&path=chanpinChakan.action";
				   document.formset.submit();
				   //window.location.href="<%=path %>/chanpinSet.action?id="+id+"&isurl=1"+"&"+property+"="+value;
               }
           }
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()" leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				<%--<form action="<%=path %>/chanpinSearchChakan.action" name="formSearch" id="formSearch" method="post">
...
地址:<input type="text" name="dizhi" id="dizhi" size="15"/>&nbsp;&nbsp;
				...
				<tr bgcolor="#E7E7E7">
					<td height="14" colspan="6" background="<%=path %>/img/tbg.gif">&nbsp;&nbsp;特色产品管理</td>
				</tr>
				<%--colunmRemarks--%>
				<tr align="center" bgcolor="#FAFAF1" height="22">
				 ...
				 <s:iterator value="#request.page.data" id="chanpin" status="s"> 
				 <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					<td bgcolor="#FFFFFF" align="center">
					 ...
					 <%--colunmNameValue--%>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#chanpin.biaoti.length()>16">
							<s:property value="#chanpin.biaoti.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#chanpin.biaoti"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#chanpin.neirong.length()>16">
							<s:property value="#chanpin.neirong.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#chanpin.neirong"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#chanpin.dizhi.length()>16">
							<s:property value="#chanpin.dizhi.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#chanpin.dizhi"/>
						</s:else>
					</td>

					<td width='90' align='center'>
						<a href='<%=path %>/<s:property value="#chanpin.image"/>' target='_blank'>
							<img
									src='<%=path %>/<s:property value="#chanpin.image"/>' width=88 height=99 border=0/>
						</a>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#chanpin.shijian.length()>16">
							<s:property value="#chanpin.shijian.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#chanpin.shijian"/>
						</s:else>
					</td>


					<td bgcolor="#FFFFFF" align="center">
						<%-- <input type="button" value="审核" onclick="chanpinSet(${chanpin.id},'shenhe','yes')"/> --%>
						<%-- <input type="button" value="取消审核" onclick="chanpinSet(${chanpin.id},'shenhe','no')"/> --%>

						<%--<input type="button" value="修改" onclick="chanpinGet(<s:property value="#chanpin.id"/>)"/>--%>
						<%--<input type="button" value="审核" onclick="chanpinSet(<s:property value="#chanpin.id"/>,'shenhe','yes')"/>--%>
						<input type="button" value="详情" onclick="chanpinDetail(<s:property value="#chanpin.id"/>)"/>
						<%--<input type="button" value="删除" onclick="chanpinDel(<s:property value="#chanpin.id"/>)"/>--%>
						<%--sonTable本表子表管理--%>

<%--
						<input type="button" value="图片管理" onclick="tupianGetBychanpin_id(<s:property value="#chanpin.id"/>)"/>
--%>
					</td>
				</tr>
				 </s:iterator>
			...
			<table width='98%'  border='0'style="margin-top:8px;margin-left: 8px;">
			  <tr>
			    <td>
			      <%--<input type="button" value="添加" style="width: 120px;" onclick="chanpinAdd()" />--%>
