...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/lianjieMana.action?d=1";
           ...
           function lianjieDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/lianjieDel.action?id="+id+"&path=lianjieChakan.action";
               }
           }
           //修改信息
           function lianjieGet(id)
           {
                   window.location.href="lianjieGet.action?id="+id;
           }
			//详细信息
			function lianjieDetail(id)
			{
				window.location.href="lianjieDetail.action?id="+id;
			}
           //图片预览
           ...
           function go(index)
           {
               ...
               document.form123.action="lianjieChakan.action"
            ...
            function lianjieSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="lianjieSet.action?id="+id+"&path=lianjieChakan.action";
				   document.formset.submit();
				   //window.location.href="<%=path %>/lianjieSet.action?id="+id+"&isurl=1"+"&"+property+"="+value;
               }
           }
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()" leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				<%--<form action="<%=path %>/lianjieSearchChakan.action" name="formSearch" id="formSearch" method="post">

					名称:<input type="text" name="name" id="name" size="15"/>&nbsp;&nbsp;
                                    链接地址:<input type="text" name="url" id="url" size="15"/>&nbsp;&nbsp;
				...
				<tr bgcolor="#E7E7E7">
					<td height="14" colspan="6" background="<%=path %>/img/tbg.gif">&nbsp;&nbsp;友情链接管理</td>
				</tr>
				<%--colunmRemarks--%>
				<tr align="center" bgcolor="#FAFAF1" height="22">
				 ...
				 <s:iterator value="#request.page.data" id="lianjie" status="s"> 
				 <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					<td bgcolor="#FFFFFF" align="center">
					 ...
					 <%--colunmNameValue--%>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#lianjie.name.length()>16">
							<s:property value="#lianjie.name.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
					...
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#lianjie.url.length()>16">
							<s:property value="#lianjie.url.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
					...
					<td bgcolor="#FFFFFF" align="center">
						<%-- <input type="button" value="审核" onclick="lianjieSet(${lianjie.id},'shenhe','yes')"/> --%>
						<%-- <input type="button" value="取消审核" onclick="lianjieSet(${lianjie.id},'shenhe','no')"/> --%>

						<%--<input type="button" value="修改" onclick="lianjieGet(<s:property value="#lianjie.id"/>)"/>--%>
						<%--<input type="button" value="审核" onclick="lianjieSet(<s:property value="#lianjie.id"/>,'shenhe','yes')"/>--%>
						<input type="button" value="详情" onclick="lianjieDetail(<s:property value="#lianjie.id"/>)"/>
						<%--<input type="button" value="删除" onclick="lianjieDel(<s:property value="#lianjie.id"/>)"/>--%>
						<%--sonTable本表子表管理--%>

<%--
						<input type="button" value="图片管理" onclick="tupianGetBylianjie_id(<s:property value="#lianjie.id"/>)"/>
--%>
					</td>
				</tr>
				 </s:iterator>
			...
			<table width='98%'  border='0'style="margin-top:8px;margin-left: 8px;">
			  <tr>
			    <td>
			      <%--<input type="button" value="添加" style="width: 120px;" onclick="lianjieAdd()" />--%>
