...
<head>
	...
	<script language="javascript">
		...
		function daochu()
		{
			var url="<%=path %>/pinglunAll.action?d=1";
		...
		//添加信息
		function pinglunAddQian()
		{
			<c:if test="${sessionScope.user==null}">
			alert("请先登录");
			</c:if>
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/pinglun/pinglunAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		...
		function go(index)
		{
			...
			document.form123.action="pinglunManaQian.action"
			document.form123.submit();
		}
		function pinglunAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="pinglunSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>评论</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/pinglunSearchQian.action">
            ...
            <input type="text" name="neirong" id="neirong" style='border:solid 1px #000000; color:#666666' size="12" />--%>
                                评论景点:
                                <select name="jingdianid" id="jingdianid" style='border:solid 1px #000000; color:#666666;'>
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<tr>
				    ...
				    <td bgcolor='#CCFFFF' align="center" >评论景点</td>
				    <td bgcolor='#CCFFFF' align="center" >评论用户</td>
				...
				<c:forEach items="${requestScope.page.data}" var="pinglun" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(pinglun.biaoti)>'16'}">
							${fn:substring(pinglun.biaoti,0,16)}...
						</c:if>
						<c:if test="${fn:length(pinglun.biaoti)<='16'}">
							${pinglun.biaoti}
						</c:if>
					...
					<%--<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(pinglun.neirong)>'16'}">
							${fn:substring(pinglun.neirong,0,16)}...
						</c:if>
						<c:if test="${fn:length(pinglun.neirong)<='16'}">
							${pinglun.neirong}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(pinglun.shijian)>'16'}">
							${fn:substring(pinglun.shijian,0,16)}...
						</c:if>
						<c:if test="${fn:length(pinglun.shijian)<='16'}">
							${pinglun.shijian}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(pinglun.jingdian.biaoti)>'16'}">
							${fn:substring(pinglun.jingdian.biaoti,0,16)}...
						</c:if>
						<c:if test="${fn:length(pinglun.jingdian.biaoti)<='16'}">
							${pinglun.jingdian.biaoti}
						</c:if>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(pinglun.users.loginname)>'16'}">
							${fn:substring(pinglun.users.loginname,0,16)}...
						</c:if>
						<c:if test="${fn:length(pinglun.users.loginname)<='16'}">
							${pinglun.users.loginname}
						</c:if>
					</td>
						...
						<td width="30" align="center">
							<a href="<%=path %>/pinglunDetailQian.action?id=${pinglun.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="pinglunAddQian()" />
							--%>
