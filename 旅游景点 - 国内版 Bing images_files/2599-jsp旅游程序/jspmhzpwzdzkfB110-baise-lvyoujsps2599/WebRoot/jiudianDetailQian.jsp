...
<head>
	...
	<script language="javascript">
		...
		//2 针对本对象添加子表信息，唯一
		            //2
            function jiudianyudingAddQian(id)
            {
                <c:if test="${sessionScope.userType ==null}">
                    alert("请先登录");
                </c:if>
                <c:if test="${sessionScope.userType !=null}">
                    var url="<%=path %>/jiudianyudingAddQian.action?id="+id;
                    window.location.href=url;
                </c:if>
            }

            function jiudianyudingGetByjiudianidQian(id)
            {
                var url="<%=path %>/jiudianyudingGetByjiudianidQian.action?jiudianid="+id;
                window.location.href=url;
            }
				...
				<ul>
					...
					<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
						...
						<tr>
							...
							<td align=left><a href=<%=path %>/${requestScope.jiudian.image } target=_blank><img
									src="<%=path %>/${requestScope.jiudian.image }" width=228 height=215 border=0></a></td>
							<%--<td width="39%" align="left">
								<img src="<%=path %>/${requestScope.jiudian.image }" alt="" width="550" height="350"/>
							</td>--%>
						</tr>
						<tr>
							<td width="39%" height="44">
							...
							<td width="39%" >
								<a href="#" onclick="down1('${requestScope.jiudian.image }','${requestScope.jiudian.image }')" style="color: red">下载</a>
							</td>
						</tr>
						...
						<tr>
							<td colspan=3 align=center>
									...
									<input type="button" value="酒店预定添加" onclick="jiudianyudingAddQian(${requestScope.jiudian.id })" />&nbsp;&nbsp;



<%--
									<input type="button" value="酒店预定查看" onclick="jiudianyudingGetByjiudianidQian(${requestScope.jiudian.id })" />
--%>
