...
<head>
	...
	<script language="javascript">
		...
		function daochu()
		{
			var url="<%=path %>/meishiAll.action?d=1";
		...
		//添加信息
		function meishiAddQian()
		{
			<c:if test="${sessionScope.user==null}">
			alert("请先登录");
			</c:if>
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/meishi/meishiAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		function meishiDel(id)
		{
			if(confirm('您确定删除吗?'))
			{
				window.location.href="<%=path %>/meishiDel.action?id="+id;
			}
		}
		//修改信息
		...
		function go(index)
		{
			...
			document.form123.action="meishiManaQian.action"
			document.form123.submit();
		}
		function meishiAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="meishiSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>美食</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/meishiSearchQian.action">
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<c:forEach items="${requestScope.page.data}" var="meishi" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(meishi.biaoti)>'16'}">
							${fn:substring(meishi.biaoti,0,16)}...
						</c:if>
						<c:if test="${fn:length(meishi.biaoti)<='16'}">
							${meishi.biaoti}
						</c:if>
					...
					<%--<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(meishi.neirong)>'16'}">
							${fn:substring(meishi.neirong,0,16)}...
						</c:if>
						<c:if test="${fn:length(meishi.neirong)<='16'}">
							${meishi.neirong}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(meishi.dizhi)>'16'}">
							${fn:substring(meishi.dizhi,0,16)}...
						</c:if>
						<c:if test="${fn:length(meishi.dizhi)<='16'}">
							${meishi.dizhi}
						</c:if>
					...
					<td width='90'>
						<a href='<%=path %>/${meishi.image}' target='_blank'>
							<img src='<%=path %>/${meishi.image}'
																							width=88 height=99
																							border=0/>
						</a>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(meishi.shijian)>'16'}">
							${fn:substring(meishi.shijian,0,16)}...
						</c:if>
						<c:if test="${fn:length(meishi.shijian)<='16'}">
							${meishi.shijian}
						</c:if>
						...
						<td width="30" align="center">
							<a href="<%=path %>/meishiDetailQian.action?id=${meishi.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="meishiAddQian()" />
							--%>
