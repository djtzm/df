...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/liuyanMana.action?d=1";
           ...
           function liuyanAdd()
           {
              var url="<%=path %>/admin/liuyan/liuyanAdd.jsp";
              window.location.href=url;
           }
           //删除信息
           function liuyanDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/liuyanDel.action?id="+id+"&path=liuyanChakanMy.action";
               }
           }
           //修改信息
           function liuyanGet(id)
           {
			...
			{
				window.location.href="liuyanDetail.action?id="+id;
			}
           //图片预览
           ...
           function go(index)
           {
               ...
               document.form123.action="liuyanChakanMy.action"
            ...
            function liuyanSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="liuyanSet.action?id="+id+"&path=liuyanChakanMy.action";
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()" leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				<form action="<%=path %>/liuyanChakanMy.action" name="formSearch" id="formSearch" method="post">
...
回复:<input type="text" name="huifu" id="huifu" size="15"/>&nbsp;&nbsp;
				...
				<tr bgcolor="#E7E7E7">
					<td height="14" colspan="6" background="<%=path %>/img/tbg.gif">&nbsp;&nbsp;留言管理</td>
				</tr>
				<tr align="center" bgcolor="#FAFAF1" height="22">
				 ...
				 <s:iterator value="#request.page.data" id="liuyan" status="s"> 
				 <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					<td bgcolor="#FFFFFF" align="center">
					 ...
					 <%--colunmNameValue--%>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#liuyan.biaoti.length()>16">
							<s:property value="#liuyan.biaoti.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#liuyan.biaoti"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#liuyan.neirong.length()>16">
							<s:property value="#liuyan.neirong.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
					...
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#liuyan.liuyanshijian.length()>16">
							<s:property value="#liuyan.liuyanshijian.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#liuyan.liuyanshijian"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#liuyan.users.loginname.length()>16">
							<s:property value="#liuyan.users.loginname.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#liuyan.users.loginname"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#liuyan.huifu.length()>16">
							<s:property value="#liuyan.huifu.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
					...
					<td bgcolor="#FFFFFF" align="center">
						<%-- <input type="button" value="审核" onclick="liuyanSet(${liuyan.id},'shenhe','yes')"/> --%>
						<%-- <input type="button" value="取消审核" onclick="liuyanSet(${liuyan.id},'shenhe','no')"/> --%>

						<%--<input type="button" value="修改" onclick="liuyanGet(<s:property value="#liuyan.id"/>)"/>--%>
						<%--<input type="button" value="审核" onclick="liuyanSet(<s:property value="#liuyan.id"/>,'shenhe','yes')"/>--%>
						<input type="button" value="详情" onclick="liuyanDetail(<s:property value="#liuyan.id"/>)"/>
						<%--<input type="button" value="删除" onclick="liuyanDel(<s:property value="#liuyan.id"/>)"/>--%>
						<%--sonTable本表子表管理--%>

<%--
						<input type="button" value="图片管理" onclick="tupianGetByliuyan_id(<s:property value="#liuyan.id"/>)"/>
--%>
					</td>
				</tr>
				 </s:iterator>
			...
			<table width='98%'  border='0'style="margin-top:8px;margin-left: 8px;">
			  <tr>
			    <td>
			      <%--<input type="button" value="添加" style="width: 120px;" onclick="liuyanAdd()" />--%>
