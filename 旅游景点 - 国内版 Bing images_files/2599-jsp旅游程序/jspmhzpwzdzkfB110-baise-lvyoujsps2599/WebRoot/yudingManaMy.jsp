...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/yudingChakanMy.action?d=1";
           ...
           function yudingDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/yudingDel.action?id="+id+"&path=yudingManaMy.action";
               }
           }
           //修改信息
           ...
           function go(index)
           {
               ...
               document.form123.action="yudingManaMy.action"
            ...
            function yudingSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="yudingSet.action?id="+id+"&path=yudingManaMy.action";
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()"  leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				...
				<form action="<%=path %>/yudingSearchMy.action" name="formSearch" id="formSearch" method="post">
