...
<head>
	...
	<script language="javascript">
		...
		//2 针对本对象添加子表信息，唯一
		            //2
            function leibiejingdianAddQian(id)
            {
                <c:if test="${sessionScope.userType ==null}">
                    alert("请先登录");
                </c:if>
                <c:if test="${sessionScope.userType !=null}">
                    var url="<%=path %>/leibiejingdianAddQian.action?id="+id;
                    window.location.href=url;
                </c:if>
            }

            function leibiejingdianGetByleibieidQian(id)
            {
                var url="<%=path %>/leibiejingdianGetByleibieidQian.action?leibieid="+id;
                window.location.href=url;
            }
            //2
            function leibiexianluAddQian(id)
            {
                <c:if test="${sessionScope.userType ==null}">
                    alert("请先登录");
                </c:if>
                <c:if test="${sessionScope.userType !=null}">
                    var url="<%=path %>/leibiexianluAddQian.action?id="+id;
                    window.location.href=url;
                </c:if>
            }

            function leibiexianluGetByleibieidQian(id)
            {
                var url="<%=path %>/leibiexianluGetByleibieidQian.action?leibieid="+id;
                window.location.href=url;
            }
...
<%--
									<input type="button" value="类别景点添加" onclick="leibiejingdianAddQian(${requestScope.leibie.id })" />&nbsp;&nbsp;
--%>



									<input type="button" value="类别景点查看" onclick="leibiejingdianGetByleibieidQian(${requestScope.leibie.id })" />


<%--
									<input type="button" value="类别旅游路线添加" onclick="leibiexianluAddQian(${requestScope.leibie.id })" />&nbsp;&nbsp;
--%>



									<input type="button" value="类别旅游路线查看" onclick="leibiexianluGetByleibieidQian(${requestScope.leibie.id })" />
