...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/dxMana.action?d=1";
           ...
           function dxAdd()
           {
              var url="<%=path %>/admin/dx/dxAdd.jsp";
              window.location.href=url;
           }
           //删除信息
           function dxDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/dxDel.action?id="+id+"&path=dxChakan.action";
               }
           }
           //修改信息
           function dxGet(id)
           {
                   window.location.href="dxGet.action?id="+id;
           }
			//详细信息
			function dxDetail(id)
			{
				window.location.href="dxDetail.action?id="+id;
			}
           //图片预览
           ...
           function go(index)
           {
               ...
               document.form123.action="dxChakan.action"
            ...
            function dxSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="dxSet.action?id="+id+"&path=dxChakan.action";
				   document.formset.submit();
				   //window.location.href="<%=path %>/dxSet.action?id="+id+"&isurl=1"+"&"+property+"="+value;
               }
           }
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()" leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				<%--<form action="<%=path %>/dxSearchChakan.action" name="formSearch" id="formSearch" method="post">

					类别:<input type="text" name="leibie" id="leibie" size="15"/>&nbsp;&nbsp;
内容:<input type="text" name="content" id="content" size="15"/>&nbsp;&nbsp;
				...
				<tr bgcolor="#E7E7E7">
					<td height="14" colspan="6" background="<%=path %>/img/tbg.gif">&nbsp;&nbsp;信息介绍管理</td>
				</tr>
				<%--colunmRemarks--%>
				<tr align="center" bgcolor="#FAFAF1" height="22">
				 ...
				 <s:iterator value="#request.page.data" id="dx" status="s"> 
				 <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					<td bgcolor="#FFFFFF" align="center">
					 ...
					 <%--colunmNameValue--%>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#dx.leibie.length()>16">
							<s:property value="#dx.leibie.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#dx.leibie"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#dx.content.length()>16">
							<s:property value="#dx.content.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#dx.content"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#dx.addtime.length()>16">
							<s:property value="#dx.addtime.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#dx.addtime"/>
						</s:else>
					</td>


					<td bgcolor="#FFFFFF" align="center">
						<%-- <input type="button" value="审核" onclick="dxSet(${dx.id},'shenhe','yes')"/> --%>
						<%-- <input type="button" value="取消审核" onclick="dxSet(${dx.id},'shenhe','no')"/> --%>

						<%--<input type="button" value="修改" onclick="dxGet(<s:property value="#dx.id"/>)"/>--%>
						<%--<input type="button" value="审核" onclick="dxSet(<s:property value="#dx.id"/>,'shenhe','yes')"/>--%>
						<input type="button" value="详情" onclick="dxDetail(<s:property value="#dx.id"/>)"/>
						<%--<input type="button" value="删除" onclick="dxDel(<s:property value="#dx.id"/>)"/>--%>
						<%--sonTable本表子表管理--%>

<%--
						<input type="button" value="图片管理" onclick="tupianGetBydx_id(<s:property value="#dx.id"/>)"/>
--%>
					</td>
				</tr>
				 </s:iterator>
			...
			<table width='98%'  border='0'style="margin-top:8px;margin-left: 8px;">
			  <tr>
			    <td>
			      <%--<input type="button" value="添加" style="width: 120px;" onclick="dxAdd()" />--%>
