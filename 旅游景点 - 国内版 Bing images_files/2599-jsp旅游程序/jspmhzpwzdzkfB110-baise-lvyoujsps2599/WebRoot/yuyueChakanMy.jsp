...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/yuyueMana.action?d=1";
           ...
           function yuyueAdd()
           {
              var url="<%=path %>/admin/yuyue/yuyueAdd.jsp";
              window.location.href=url;
           }
           //删除信息
           function yuyueDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/yuyueDel.action?id="+id+"&path=yuyueChakanMy.action";
               }
           }
           //修改信息
           function yuyueGet(id)
           {
			...
			{
				window.location.href="yuyueDetail.action?id="+id;
			}
           //图片预览
           ...
           function go(index)
           {
               ...
               document.form123.action="yuyueChakanMy.action"
            ...
            function yuyueSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="yuyueSet.action?id="+id+"&path=yuyueChakanMy.action";
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()" leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				<form action="<%=path %>/yuyueChakanMy.action" name="formSearch" id="formSearch" method="post">

					标题:<input type="text" name="biaoti" id="biaoti" size="15"/>&nbsp;&nbsp;
                                预约旅游路线:
                                <select name="xianluid" id="xianluid" >
...
<%--
                                        日期:<input type="text"  class="Wdate"  name="riqi" id="riqi"  onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd'})"  value="<%=new SimpleDateFormat("yyyy-MM-dd").format(new Date())%>"/>&nbsp;&nbsp;
--%>
                                        日期:
...
备注:<input type="text" name="beizhu" id="beizhu" size="15"/>&nbsp;&nbsp;
				...
				<tr bgcolor="#E7E7E7">
					<td height="14" colspan="6" background="<%=path %>/img/tbg.gif">&nbsp;&nbsp;预约管理</td>
				</tr>
				<tr align="center" bgcolor="#FAFAF1" height="22">
				    <td width="2%">序号</td>
					<%--colunmRemarks--%>
				    <td width="5%">标题</td>
				    <td width="4%">预约旅游路线</td>
				    <td width="4%">预约用户</td>
				    <td width="5%">日期</td>
				    <td width="5%">姓名</td>
				    <td width="4%">电话</td>
				 ...
				 <s:iterator value="#request.page.data" id="yuyue" status="s"> 
				 <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					<td bgcolor="#FFFFFF" align="center">
					 ...
					 <%--colunmNameValue--%>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#yuyue.biaoti.length()>16">
							<s:property value="#yuyue.biaoti.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#yuyue.biaoti"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#yuyue.xianlu.biaoti.length()>16">
							<s:property value="#yuyue.xianlu.biaoti.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#yuyue.xianlu.biaoti"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#yuyue.users.loginname.length()>16">
							<s:property value="#yuyue.users.loginname.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#yuyue.users.loginname"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#yuyue.riqi.length()>16">
							<s:property value="#yuyue.riqi.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#yuyue.riqi"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#yuyue.xingming.length()>16">
							<s:property value="#yuyue.xingming.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#yuyue.xingming"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#yuyue.dianhua.length()>16">
							<s:property value="#yuyue.dianhua.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
							<s:property value="#yuyue.dianhua"/>
						</s:else>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<s:if test="#yuyue.beizhu.length()>16">
							<s:property value="#yuyue.beizhu.substring(0,16)+'...'"/>
						</s:if>
						<s:else>
					...
					<td bgcolor="#FFFFFF" align="center">
						<%-- <input type="button" value="审核" onclick="yuyueSet(${yuyue.id},'shenhe','yes')"/> --%>
						<%-- <input type="button" value="取消审核" onclick="yuyueSet(${yuyue.id},'shenhe','no')"/> --%>

						<%--<input type="button" value="修改" onclick="yuyueGet(<s:property value="#yuyue.id"/>)"/>--%>
						<%--<input type="button" value="审核" onclick="yuyueSet(<s:property value="#yuyue.id"/>,'shenhe','yes')"/>--%>
						<input type="button" value="详情" onclick="yuyueDetail(<s:property value="#yuyue.id"/>)"/>
						<%--<input type="button" value="删除" onclick="yuyueDel(<s:property value="#yuyue.id"/>)"/>--%>
						<%--sonTable本表子表管理--%>

<%--
						<input type="button" value="图片管理" onclick="tupianGetByyuyue_id(<s:property value="#yuyue.id"/>)"/>
--%>
					</td>
				</tr>
				 </s:iterator>
			...
			<table width='98%'  border='0'style="margin-top:8px;margin-left: 8px;">
			  <tr>
			    <td>
			      <%--<input type="button" value="添加" style="width: 120px;" onclick="yuyueAdd()" />--%>
