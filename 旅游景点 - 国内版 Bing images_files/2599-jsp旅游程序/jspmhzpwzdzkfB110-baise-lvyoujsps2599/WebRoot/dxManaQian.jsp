...
<head>
	...
	<script language="javascript">
		...
		function dxAddQian()
		{
			...
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/dx/dxAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		function dxDel(id)
		{
			if(confirm('您确定删除吗?'))
			{
				window.location.href="<%=path %>/dxDel.action?id="+id;
			}
		}
		//修改信息
		function dxGet(id)
		...
		function go(index)
		{
			...
			document.form123.action="dxManaQian.action"
			document.form123.submit();
		}
		function dxAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="dxSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>信息介绍</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/dxSearchQian.action">

				            类别:
            <input type="text" name="leibie" id="leibie" style='border:solid 1px #000000; color:#666666' size="12" />
            <%--内容:
            <input type="text" name="content" id="content" style='border:solid 1px #000000; color:#666666' size="12" />--%>
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<tr>
					...
					<%--colunmRemarks--%>
									    <td bgcolor='#CCFFFF' align="center" >类别</td>
				...
				<c:forEach items="${requestScope.page.data}" var="dx" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(dx.leibie)>'16'}">
							${fn:substring(dx.leibie,0,16)}...
						</c:if>
						<c:if test="${fn:length(dx.leibie)<='16'}">
							${dx.leibie}
						</c:if>
					...
					<%--<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(dx.content)>'16'}">
							${fn:substring(dx.content,0,16)}...
						</c:if>
						<c:if test="${fn:length(dx.content)<='16'}">
							${dx.content}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(dx.addtime)>'16'}">
							${fn:substring(dx.addtime,0,16)}...
						</c:if>
						<c:if test="${fn:length(dx.addtime)<='16'}">
							${dx.addtime}
						</c:if>
						...
						<td width="30" align="center">
							<a href="<%=path %>/dxDetailQian.action?id=${dx.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="dxAddQian()" />
							--%>
