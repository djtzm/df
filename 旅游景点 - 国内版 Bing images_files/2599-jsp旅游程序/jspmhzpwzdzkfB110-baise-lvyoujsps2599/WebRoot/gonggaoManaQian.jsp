...
<head>
	...
	<script language="javascript">
		...
		function daochu()
		{
			var url="<%=path %>/gonggaoAll.action?d=1";
		...
		//添加信息
		function gonggaoAddQian()
		{
			<c:if test="${sessionScope.user==null}">
			alert("请先登录");
			</c:if>
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/gonggao/gonggaoAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		...
		function go(index)
		{
			...
			document.form123.action="gonggaoManaQian.action"
			document.form123.submit();
		}
		function gonggaoAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="gonggaoSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>公告新闻</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/gonggaoSearchQian.action">
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<c:forEach items="${requestScope.page.data}" var="gonggao" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(gonggao.biaoti)>'16'}">
							${fn:substring(gonggao.biaoti,0,16)}...
						</c:if>
						<c:if test="${fn:length(gonggao.biaoti)<='16'}">
							${gonggao.biaoti}
						</c:if>
					...
					<%--<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(gonggao.neirong)>'16'}">
							${fn:substring(gonggao.neirong,0,16)}...
						</c:if>
						<c:if test="${fn:length(gonggao.neirong)<='16'}">
							${gonggao.neirong}
						</c:if>
					...
					<td width='90'>
						<a href='<%=path %>/${gonggao.tupian}' target='_blank'>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(gonggao.shijian)>'16'}">
							${fn:substring(gonggao.shijian,0,16)}...
						</c:if>
						<c:if test="${fn:length(gonggao.shijian)<='16'}">
							${gonggao.shijian}
						</c:if>
						...
						<td width="30" align="center">
							<a href="<%=path %>/gonggaoDetailQian.action?id=${gonggao.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="gonggaoAddQian()" />
							--%>
