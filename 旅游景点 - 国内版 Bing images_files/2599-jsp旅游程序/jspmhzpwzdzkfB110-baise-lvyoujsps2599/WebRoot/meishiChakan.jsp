...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/meishiMana.action?d=1";
           ...
           function meishiAdd()
           {
              var url="<%=path %>/admin/meishi/meishiAdd.jsp";
              window.location.href=url;
           }
           //删除信息
           function meishiDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/meishiDel.action?id="+id+"&path=meishiChakan.action";
               }
           }
           //修改信息
           ...
           function go(index)
           {
               ...
               document.form123.action="meishiChakan.action"
            ...
            function meishiSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="meishiSet.action?id="+id+"&path=meishiChakan.action";
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()" leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				<%--<form action="<%=path %>/meishiSearchChakan.action" name="formSearch" id="formSearch" method="post">
...
地址:<input type="text" name="dizhi" id="dizhi" size="15"/>&nbsp;&nbsp;
				...
				<tr align="center" bgcolor="#FAFAF1" height="22">
				 ...
				 <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					...
					<td bgcolor="#FFFFFF" align="center">
						<%-- <input type="button" value="审核" onclick="meishiSet(${meishi.id},'shenhe','yes')"/> --%>
						<%-- <input type="button" value="取消审核" onclick="meishiSet(${meishi.id},'shenhe','no')"/> --%>

						<%--<input type="button" value="修改" onclick="meishiGet(<s:property value="#meishi.id"/>)"/>--%>
						<%--<input type="button" value="审核" onclick="meishiSet(<s:property value="#meishi.id"/>,'shenhe','yes')"/>--%>
...
<%--
						<input type="button" value="图片管理" onclick="tupianGetBymeishi_id(<s:property value="#meishi.id"/>)"/>
--%>
					</td>
				</tr>
				 </s:iterator>
