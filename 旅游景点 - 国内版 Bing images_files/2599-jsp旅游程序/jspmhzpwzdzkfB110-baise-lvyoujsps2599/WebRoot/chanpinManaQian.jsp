...
<head>
	...
	<script language="javascript">
		...
		function daochu()
		{
			var url="<%=path %>/chanpinAll.action?d=1";
		...
		//添加信息
		function chanpinAddQian()
		{
			<c:if test="${sessionScope.user==null}">
			alert("请先登录");
			</c:if>
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/chanpin/chanpinAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		function chanpinDel(id)
		{
			if(confirm('您确定删除吗?'))
			{
				window.location.href="<%=path %>/chanpinDel.action?id="+id;
			}
		}
		//修改信息
		...
		function go(index)
		{
			...
			document.form123.action="chanpinManaQian.action"
			document.form123.submit();
		}
		function chanpinAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="chanpinSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>特色产品</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/chanpinSearchQian.action">
            ...
            地址:
            <input type="text" name="dizhi" id="dizhi" style='border:solid 1px #000000; color:#666666' size="12" />
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<tr>
				    ...
				    <td bgcolor='#CCFFFF' align="center" >图片</td>
				...
				<c:forEach items="${requestScope.page.data}" var="chanpin" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(chanpin.biaoti)>'16'}">
							${fn:substring(chanpin.biaoti,0,16)}...
						</c:if>
						<c:if test="${fn:length(chanpin.biaoti)<='16'}">
							${chanpin.biaoti}
						</c:if>
					...
					<%--<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(chanpin.neirong)>'16'}">
							${fn:substring(chanpin.neirong,0,16)}...
						</c:if>
						<c:if test="${fn:length(chanpin.neirong)<='16'}">
							${chanpin.neirong}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(chanpin.dizhi)>'16'}">
							${fn:substring(chanpin.dizhi,0,16)}...
						</c:if>
						<c:if test="${fn:length(chanpin.dizhi)<='16'}">
							${chanpin.dizhi}
						</c:if>
					...
					<td width='90'>
						<a href='<%=path %>/${chanpin.image}' target='_blank'>
							<img src='<%=path %>/${chanpin.image}'
																							width=88 height=99
																							border=0/>
						</a>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(chanpin.shijian)>'16'}">
							${fn:substring(chanpin.shijian,0,16)}...
						</c:if>
						<c:if test="${fn:length(chanpin.shijian)<='16'}">
							${chanpin.shijian}
						</c:if>
						...
						<td width="30" align="center">
							<a href="<%=path %>/chanpinDetailQian.action?id=${chanpin.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="chanpinAddQian()" />
							--%>
