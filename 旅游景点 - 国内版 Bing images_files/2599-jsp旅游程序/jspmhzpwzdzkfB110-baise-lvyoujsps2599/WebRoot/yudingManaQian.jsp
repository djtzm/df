...
<head>
	...
	<script language="javascript">
		...
		function daochu()
		{
			var url="<%=path %>/yudingAll.action?d=1";
		...
		//添加信息
		function yudingAddQian()
		{
			<c:if test="${sessionScope.user==null}">
			alert("请先登录");
			</c:if>
			<c:if test="${sessionScope.user!=null}">
			var url="<%=path %>/qiantai/yuding/yudingAddQian.jsp";
			window.location.href=url;
			</c:if>
		}
		//删除信息
		...
		function go(index)
		{
			...
			document.form123.action="yudingManaQian.action"
			document.form123.submit();
		}
		function yudingAll(id,property,value)
		{
			if(confirm('您确定操作吗?'))
			{
				...
				document.formset.action="yudingSet.action?id="+id
...
<div class="wrapper">
	<div class="news_con">
		<div class="news_list_con clearfix">
			...
			<h2>预定</h2>
			<form name="form1" id="form1" method="post" action="<%=path %>/yudingSearchQian.action">

				            标题:
            <input type="text" name="biaoti" id="biaoti" style='border:solid 1px #000000; color:#666666' size="12" />
                                预定酒店:
                                <select name="jiudianid" id="jiudianid" style='border:solid 1px #000000; color:#666666;'>
...
--%>
			...
			<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
				...
				<tr>
				    ...
				    <td bgcolor='#CCFFFF' align="center" >预定酒店</td>
				    <td bgcolor='#CCFFFF' align="center" >预定用户</td>
				...
				<c:forEach items="${requestScope.page.data}" var="yuding" varStatus="s">
					...
					<tr>
						...
						<c:if test="${fn:length(yuding.biaoti)>'16'}">
							${fn:substring(yuding.biaoti,0,16)}...
						</c:if>
						<c:if test="${fn:length(yuding.biaoti)<='16'}">
							${yuding.biaoti}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(yuding.jiudian.biaoti)>'16'}">
							${fn:substring(yuding.jiudian.biaoti,0,16)}...
						</c:if>
						<c:if test="${fn:length(yuding.jiudian.biaoti)<='16'}">
							${yuding.jiudian.biaoti}
						</c:if>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(yuding.users.loginname)>'16'}">
							${fn:substring(yuding.users.loginname,0,16)}...
						</c:if>
						<c:if test="${fn:length(yuding.users.loginname)<='16'}">
							${yuding.users.loginname}
						</c:if>
					</td>
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(yuding.riqi)>'16'}">
							${fn:substring(yuding.riqi,0,16)}...
						</c:if>
						<c:if test="${fn:length(yuding.riqi)<='16'}">
							${yuding.riqi}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(yuding.xingming)>'16'}">
							${fn:substring(yuding.xingming,0,16)}...
						</c:if>
						<c:if test="${fn:length(yuding.xingming)<='16'}">
							${yuding.xingming}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(yuding.dianhua)>'16'}">
							${fn:substring(yuding.dianhua,0,16)}...
						</c:if>
						<c:if test="${fn:length(yuding.dianhua)<='16'}">
							${yuding.dianhua}
						</c:if>
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${fn:length(yuding.beizhu)>'16'}">
							${fn:substring(yuding.beizhu,0,16)}...
						</c:if>
						<c:if test="${fn:length(yuding.beizhu)<='16'}">
							${yuding.beizhu}
						</c:if>
						...
						<td width="30" align="center">
							<a href="<%=path %>/yudingDetailQian.action?id=${yuding.id}">
								详细
							</a>
						</td>
					</tr>
				   ...
				   style="border-collapse:collapse" class="newsline">
					<tr>
						<td align="center" bgcolor="CCFFFF">
							<%--
							<input type="button" value="添加" style="width: 120px;" onclick="yudingAddQian()" />
							--%>
