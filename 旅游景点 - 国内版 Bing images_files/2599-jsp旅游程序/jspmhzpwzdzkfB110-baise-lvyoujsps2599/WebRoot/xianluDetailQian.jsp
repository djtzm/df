...
<head>
	...
	<script language="javascript">
		...
		//2 针对本对象添加子表信息，唯一
		            //2
            function xianluyuyueAddQian(id)
            {
                <c:if test="${sessionScope.userType ==null}">
                    alert("请先登录");
                </c:if>
                <c:if test="${sessionScope.userType !=null}">
                    var url="<%=path %>/xianluyuyueAddQian.action?id="+id;
                    window.location.href=url;
                </c:if>
            }

            function xianluyuyueGetByxianluidQian(id)
            {
                var url="<%=path %>/xianluyuyueGetByxianluidQian.action?xianluid="+id;
                window.location.href=url;
            }
				...
				<ul>
					...
					<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
						...
						<tr>
							...
							<td width="39%" >
								${requestScope.xianlu.leibie.mingcheng }
							</td>
						</tr>
						<tr>
							...
							<td align=left><a href=<%=path %>/${requestScope.xianlu.image } target=_blank><img
									src="<%=path %>/${requestScope.xianlu.image }" width=228 height=215 border=0></a></td>
                        ...
                        <tr>
                            ...
                            <td width="39%" >
                                ${requestScope.xianlu.users.loginname }
                            </td>
                        </tr>
						...
						<tr>
							<td colspan=3 align=center>
									...
									<input type="button" value="旅游路线预约添加" onclick="xianluyuyueAddQian(${requestScope.xianlu.id })" />&nbsp;&nbsp;



<%--
									<input type="button" value="旅游路线预约查看" onclick="xianluyuyueGetByxianluidQian(${requestScope.xianlu.id })" />
--%>
