...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script language="javascript">
            ...
            function daochu()
           {
              var url="<%=path %>/jingdianChakan.action?d=1";
           ...
           function jingdianDel(id)
           {
               if(confirm('您确定删除吗?'))
               {
                   window.location.href="<%=path %>/jingdianDel.action?id="+id+"&path=jingdianMana.action";
               }
           }
           //修改信息
           ...
           function go(index)
           {
               ...
               document.form123.action="jingdianMana.action"
            ...
            function jingdianSet(id,property,value)
           {
               if(confirm('您确定操作吗?'))
               {
				   ...
				   document.formset.action="jingdianSet.action?id="+id+"&path=jingdianMana.action";
...
/*			function tupianGetByxinwen_id(xinwen_id)
	...
	<body onload="init()"  leftmargin="2" topmargin="2" background='<%=path %>/img/allbg.gif'>
			<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px">
				...
				<form action="<%=path %>/jingdianMana.action" name="formSearch" id="formSearch" method="post">
