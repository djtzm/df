...
public class meishiAction extends ActionSupport
{
    /*2列属性*/
    ...
    public String meishiAdd() {
        ...
        if(!isEmpty(biaoti))
        {
            meishi.setBiaoti(biaoti);
        }
        if(!isEmpty(neirong))
        {
            meishi.setNeirong(neirong);
        }
        if(!isEmpty(dizhi))
        {
            meishi.setDizhi(dizhi);
        }
        if(!isEmpty(image))
        {
            meishi.setImage(image);
        }
        if(!isEmpty(shijian))
        {
            meishi.setShijian(shijian);
        }


        meishiDAO.save(meishi);
    ...
    //set后台信息修改方法实现代码...................
    public String meishiSet()
    {
        meishi meishi=meishiDAO.findById(id);
        /*10setbody*/
    ...
    public String meishiSet2()
    {
        ...
        meishi meishisession = (meishi) session.get("user");
        if (meishisession!=null)
        {
            meishi meishidatabase = meishiDAO.findById(meishisession.getId());
            session.put("user", meishidatabase);
        }
    ...
    //Mana 后台信息管理列表方法实现代码.............
    public String meishiMana()
    {
        String sql="from meishi where 1=1 ";
        ...
        {
            meishi meishi=(meishi)meishiList.get(i);
            //示例代码
        ...
        request.put("meishiList", meishiList);
        ...
        int toIndex = Math.min(fromIndex + hsgpagesize, meishiList.size());
        List meishiList1 = meishiList.subList(fromIndex, toIndex);

        p.setIndex(index);//设置页数
        p.setPageSize(hsgpagesize);
        p.setTotle(meishiList.size());//设置总共的条数
        p.setData(meishiList1);//设置数据
    ...
    public String meishiManaQianSort()
    {
        String sql="from meishi where 1=1 ";
    ...
    public String meishiManaMy()
    {
        ...
        Pagination p = new Pagination();//创建 分页对象
        //int hsgpagesize=p.getPageSize();
        int hsgpagesize=meishiList.size()+1;
        //hsgpagesize=10;
        int fromIndex = (index - 1) * hsgpagesize;
        int toIndex = Math.min(fromIndex + hsgpagesize, meishiList.size());
        List meishiList1 = meishiList.subList(fromIndex, toIndex);
    ...
    public String meishiManaMyQian()
    {
        String sql="from meishi where 1=1 ";
    ...
    public String meishiSearchQian()
    {
        String sql="from meishi where 1=1 ";
        if(!isEmpty(id))
        {
    ...
    //del 按id删除信息方法.............
    public String meishiDel()
    {
        meishi meishi=meishiDAO.findById(id);
        meishiDAO.delete(meishi);

        this.setMessage("操作成功");
        //this.setPath("meishiMana.action");
    ...
    public String meishiDetailQian()
    {
        meishi meishi=meishiDAO.findById(id);
