...
public class jiudianAction extends ActionSupport
{
    ...
    public String jiudianAdd() {
        ...
        if(!isEmpty(biaoti))
        {
            jiudian.setBiaoti(biaoti);
        }
        if(!isEmpty(neirong))
        {
            jiudian.setNeirong(neirong);
        }
        if(!isEmpty(dizhi))
        {
        ...
        if(!isEmpty(image))
        {
            jiudian.setImage(image);
        }
        if(!isEmpty(shijian))
        {
            jiudian.setShijian(shijian);
        }
    ...
    //set后台信息修改方法实现代码...................
    public String jiudianSet()
    {
        jiudian jiudian=jiudianDAO.findById(id);
        /*10setbody*/
    ...
    public String jiudianSet2()
    {
        ...
        jiudian jiudiansession = (jiudian) session.get("user");
        if (jiudiansession!=null)
        {
            jiudian jiudiandatabase = jiudianDAO.findById(jiudiansession.getId());
            session.put("user", jiudiandatabase);
        }
    ...
    {
        String sql="from jiudian where 1=1 ";
        ...
        {
            jiudian jiudian=(jiudian)jiudianList.get(i);
            //示例代码
        ...
        int toIndex = Math.min(fromIndex + hsgpagesize, jiudianList.size());
        List jiudianList1 = jiudianList.subList(fromIndex, toIndex);

        p.setIndex(index);//设置页数
        p.setPageSize(hsgpagesize);
        p.setTotle(jiudianList.size());//设置总共的条数
        p.setData(jiudianList1);//设置数据
    ...
    public String jiudianManaQianSort()
    {
        String sql="from jiudian where 1=1 ";
    ...
    public String jiudianManaMy()
    {
        ...
        Pagination p = new Pagination();//创建 分页对象
        //int hsgpagesize=p.getPageSize();
        int hsgpagesize=jiudianList.size()+1;
        //hsgpagesize=10;
        int fromIndex = (index - 1) * hsgpagesize;
        int toIndex = Math.min(fromIndex + hsgpagesize, jiudianList.size());
        List jiudianList1 = jiudianList.subList(fromIndex, toIndex);
    ...
    public String jiudianManaMyQian()
    {
        String sql="from jiudian where 1=1 ";
    ...
    public String jiudianSearchQian()
    {
        String sql="from jiudian where 1=1 ";
        if(!isEmpty(id))
        {
    ...
    public String jiudianGet()
    {
...

        {
            ...
            //SonListGet
                    ...
                    {
                        String sql="from yuding where jiudianid="+id;
